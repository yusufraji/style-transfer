...
def train(args):
    ...
    for e in range(args.epochs):
        ...
        for batch_id, (x, _) in enumerate(train_loader):
            ...
            agg_style_loss += agg_style_loss.item()
...
def stylize(args):
    ...
    if args.model.endswith(".onnx"):
        ...
        with torch.no_grad():
            ...
            # remove saved deprecated running_* keys in InstanceNorm from the
            # checkpoint
...
def stylize_onnx_caffe2(content_image, args):
    ...
    prepared_backend = onnx_caffe2.backend.prepared(
        model, device="CUDA" if args.device else "CPU"
    )
...
def main():
    ...
    train_arg_parser.add_argument(
        ...
        type=str,
        required=True,
        help="path to training dataset, the path should point to a folder containing another folder with all the training images",
    )
    train_arg_parser.add_argument(
        "--style-image",
        type=str,
    ...
    train_arg_parser.add_argument(
        ...
        type=int,
        default=2000,
        help="umber of batches after which a checkpoint of the trained model will be created, default is 2000",
    )

    eval_arg_parser = subparsers.add_parser(
        "eval", help="parser for evaluating/stylizing arguments"
    )
    ...
    eval_arg_parser.add_argument(
        ...
        type=str,
        required=True,
        help="saved model to be used for stylizing the image. If file end in .pty - PyTorch path is used, if in .onnx - Caffe2 path is used",
    )
    eval_arg_parser.add_argument(
        "--cuda",
        type=int,
    ...
    eval_arg_parser.add_argument(
        "--export-onnx", type=str, help="export ONNX model to a given file"
    )
    ...
    if args.cuda and not torch.cuda.is_available():
        print("ERROR: cuda is not available, try running on CPU instead")
        sys.exit(1)
